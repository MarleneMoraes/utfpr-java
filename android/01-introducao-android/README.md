# 01 | Introdução a Plataforma Android
## Análise do Mercado
### Conteúdo
- [*Smartphone OS sales market share](https://www.kantarworldpanel.com/global/smartphone-os-market-share/)
- [Pesquisa Salarial de Programadores Brasileiros - Possui Limitações Metodológicas](https://pesquisa.codigofonte.com.br/2024/area/mobile)

### Material Complementar
- [Como foi a última década no mercado de smartphones](https://www.targethd.net/como-foi-a-ultima-decada-no-mercado-de-smartphones-entre-2010-e-2019/)
- [Sua loja precisa de um aplicativo próprio?](https://dcomercio.com.br/publicacao/s/sua-loja-precisa-de-um-aplicativo-proprio)
- [Por que as telas dos celulares cresceram tanto nos últimos 10 anos?](https://www.terra.com.br/byte/por-que-as-telas-dos-celulares-cresceram-tanto-nos-ultimos-10-anos,6a58fe5c3e36ef1d8d9e2fa6547224beig6hvuwt.html#google_vignette)
- [Entenda o que faz o mercado brasileiro de aplicativos estar em alta](https://olhardigital.com.br/2020/09/01/pro/saiba-por-que-o-mercado-brasileiro-de-aplicativos-esta-com-tudo/)
- [Mobile Time - Dados do Mercado Brasileiro](https://www.mobiletime.com.br/dados-de-mercado/)
- [Impacto econômico e social do Android no Brasil](https://www.bain.com/contentassets/a9200a057a0241b8963c05a9b09e33fe/impactos-do-android-no-brasil.pdf)
- [Android colaborou com R$ 88 bilhões para o PIB brasileiro em 2022](https://www.mobiletime.com.br/noticias/29/11/2023/android-colaborou-com-r-88-bilhoes-para-o-pib-brasileiro-em-2022/)
- [Monetizando um aplicativo: 8 formas de lucrar com o seu app](https://ejcm.com.br/como-lucrar-com-o-seu-app/)
- [Aplicativo nativo, híbrido ou PWA: qual melhor escolha para a minha empresa?](https://imaginedone.com.br/artigos/web-e-mobile/aplicativo-nativo-hibrido-pwa/)
- [Native vs Hybrid: Which Mobile App Platform Should You Choose?](https://www.codemotion.com/magazine/frontend/mobile-dev/native-vs-hybrid-which-mobile-app-platform-should-you-choose/)

## Visão Geral
### Conteúdo
- [Arquitetura da Plataforma Android](https://developer.android.com/guide/platform?hl=pt-br)

### Material Complementar
- [O que é o Android](https://www.android.com/intl/pt-BR_br/what-is-android/)
- [A história do Android](https://www.oficinadanet.com.br/post/13939-a-historia-do-android)
- [Fundamentos de aplicativos](https://developer.android.com/guide/components/fundamentals?hl=pt-br)
- [Dashboards - Análise dos Sistemas Android](https://developer.android.com/about/dashboards?hl=pt-br)

## Distribuições
- [Android Open Source Project (AOSP)](https://source.android.com/?hl=pt-br)
- [Stock Android vs Android One vs Android Go](https://www.androidauthority.com/stock-android-vs-android-one-vs-android-go-860020/)

## Projeto Treble
- [Separação da interface das fabricantes do sistema para diminuir o tempo de atualizações](https://www.ajudandroid.com.br/projeto-treble-separa-a-interface-das-fabricantes-do-sistema-para-diminuir-o-tempo-de-atualizacoes-no-android-o/)

## Links Básicos
### Conteúdo
- [Google - Android Developers](https://developer.android.com/?hl=pt-br)

### Material Complementar
- [Google Play](https://play.google.com/store/games?device=windows&pli=1)

## Outras Iniciativas Google
- [Fuchsia](https://fuchsia.dev/)
- [Flutter](https://flutter.dev/)
- [Progressive Web Apps](https://web.dev/explore/progressive-web-apps?hl=pt-br)

## Complementos
- [APKMirror](https://www.apkmirror.com/)

# 02 | Ambiente de Desenvolvimento Android Studio
## Visão Geral
### Conteúdo
- [Download Android Studio com SDK Tools](https://developer.android.com/studio?hl=en)

### Material Complementar
- [Conheça o Android Studio](https://developer.android.com/studio/intro?hl=pt-br)

## Guias sobre a Ferramenta
- [Configurar o Android Studio](https://developer.android.com/studio/intro/studio-config?hl=pt-br)
- [Atualizar o IDE e o SDK Tools](https://developer.android.com/studio/intro/update)
- [Migrar para o Android Studio](https://developer.android.com/studio/intro/migrate?hl=pt-br)

## SDK Tools (ADT)
- [Ferramentas de linha de comando](https://developer.android.com/studio/command-line?hl=pt-br)
- [Download apenas do SDK (ADT)](https://developer.android.com/studio?hl=en#:~:text=Command%20line%20tools%20only)

## Desenvolvimento
- [Visão geral de projetos](https://developer.android.com/studio/projects?hl=pt-br)
- [Configure sua versão](https://developer.android.com/studio/build/?hl=pt-br)
- [Depure seu aplicativo](https://developer.android.com/studio/debug?hl=pt-br)

## Otimização
- [Otimizar a velocidade do seu build](https://developer.android.com/build/optimize-your-build?hl=pt-br)

## Resolução de Problemas
- [Resolver problemas do Android Studio](https://developer.android.com/studio/troubleshoot.html?hl=pt-br)
- [Configurar o ambiente de desenvolvimento integrado para máquinas com pouca memória](https://developer.android.com/studio/intro/studio-config?hl=pt-br#low_memory)
- [Improving Android Studio Performance on Memory-Constrained Machines](https://www.youtube.com/watch?v=3TENdS9Qqcc)

## Requisitos para Publicação do App
- [Atender ao requisito de nível desejado da API do Google Play](https://developer.android.com/google/play/requirements/target-sdk?hl=pt-br)
- [Overview of all Android API Levels](https://apilevels.com/)

## Configurações de Preview
- [Referência de atributos de ferramentas](https://developer.android.com/studio/write/tool-attributes?hl=pt-br)

# 03 | Executando um Aplicativo no Emulador Android
## Guias sobre a Ferramenta
- [Executar apps no Android Emulator](https://developer.android.com/studio/run/emulator?hl=pt-br)
- [Criar e gerenciar dispositivos virtuais](https://developer.android.com/studio/run/managing-avds?hl=pt-br)
- [Iniciar o emulador pela linha de comando](https://developer.android.com/studio/run/emulator-commandline?hl=pt-br)
- [Criar e executar seu aplicativo](https://developer.android.com/studio/run/index.html)

## Aceleração de Hardware
- [Configurar a aceleração de hardware para o Android Emulator](https://developer.android.com/studio/run/emulator-acceleration?hl=pt-br)

## Identificar Hardware do Aparelho
- [Aplicativo CPU-Z](https://play.google.com/store/apps/details?id=com.cpuid.cpu_z&hl=pt_BR&pli=1)

## Resolução de Problemas
- [Resolver problemas conhecidos com o Android Emulator](https://developer.android.com/studio/run/emulator-troubleshooting?hl=pt-br)
- [Fixed Error Emulator process was killed](https://www.youtube.com/watch?v=AOK9ZxiBOGg)

# 04 | Executando um Aplicativo no Dispositivo
## Guias sobre a Ferramenta
- [Executar apps em um dispositivo de hardware](https://developer.android.com/studio/run/device?hl=pt-br)
- [Instalar drivers USB do Fabricante do Dispostivo](https://developer.android.com/studio/run/oem-usb.html)
- [Obter o driver USB do Google para o Dispositivo Móvel](https://developer.android.com/studio/run/win-usb)